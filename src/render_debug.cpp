...
struct RenderDebugPanelSystem : System<> {
    static float draw_slider(const std::string& label, float x, float y,
        ...
        draw_text_ex(get_font(), label.c_str(), {x, y - 20}, 18, FONT_SPACING,
                     Color{200, 200, 200, 255});

        std::string val_text = fmt::format("{:.2f}", val);
        draw_text_ex(get_font(), val_text.c_str(), {x + w + 8, y - 2}, 18,
                     FONT_SPACING, Color{255, 255, 100, 255});

        draw_rect(x, y, w, h, Color{60, 60, 70, 255});

        float t = (val - min_val) / (max_val - min_val);
        draw_rect(x, y, w * t, h, Color{80, 140, 220, 200});
    ...
    void once(float) override {
        ...
        draw_rect(px, py, pw, ph, Color{15, 15, 25, 230});
        draw_rect_lines(px, py, pw, ph, Color{100, 100, 120, 255});

        draw_text_ex(get_font(), "Debug [`]", {px + 10, py + 8}, 20,
                     FONT_SPACING, Color{255, 200, 80, 255});
        ...
        draw_text_ex(get_font(), info.c_str(), {sx, py + 200}, 16, FONT_SPACING,
